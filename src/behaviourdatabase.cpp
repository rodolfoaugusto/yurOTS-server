...
bool BehaviourDatabase::loadActions(ScriptReader& script, NpcBehaviour* behaviour)
{
	while (true) {
		...
		if (script.Token == STRING) {
			...
			if (identifier == "idle") {
			...
			} else if (identifier == "finishtask") {
				action->type = BEHAVIOUR_TYPE_FINISHTASK;
				...
				searchType = BEHAVIOUR_PARAMETER_THREE;
			} else if (identifier == "newbieitems") {
				action->type = BEHAVIOUR_TYPE_NEWBIEITEMS;
				searchType = BEHAVIOUR_PARAMETER_ONE;
			}else if (identifier == "rewardtask") {
				action->type = BEHAVIOUR_TYPE_REWARDTASK;
				searchType = BEHAVIOUR_PARAMETER_THREE;
			} else {
...
void BehaviourDatabase::checkAction(const NpcBehaviourAction* action, Player* player, const std::string& message)
{
	...
	case BEHAVIOUR_TYPE_FINISHTASK: {
		int32_t activeTask;

		player->getStorageValue(18000, activeTask);
		player->addStorageValue((19000 + activeTask), 1);
		player->addStorageValue(18000, 0);
		break;
	} 	
	...
	case BEHAVIOUR_TYPE_REWARDTASK: {
		int32_t money = evaluate(action->expression, player, message);
		int32_t experience = evaluate(action->expression2, player, message);
		int32_t points = evaluate(action->expression3, player, message);
		...
		if (points > 0){
			player->setBankBalance(player->getBankBalance() + points);
		}
		
		break;
	}
	case BEHAVIOUR_TYPE_NEWBIEITEMS: {
			Item* container = Item::CreateItem(2854);
			if (!container) {
				std::cout << "[Error - BehaviourDatabase::checkAction]: CreateContainer - failed to create container item" << std::endl;
				break;
			}
			...
			int32_t newVocation = evaluate(action->expression, player, message);
			if (newVocation == 1) {
				Item* wand = Item::CreateItem(3074, 1);
				realContainer->internalAddThing(wand);
			} else if (newVocation == 2) {
				Item* rod = Item::CreateItem(3066, 1);
				realContainer->internalAddThing(rod);
			} else if (newVocation == 3) {
				Item* spear = Item::CreateItem(3277, 3);
				realContainer->internalAddThing(spear);
			} else if (newVocation == 4) {
				Item* axe = Item::CreateItem(3329, 1);
				realContainer->internalAddThing(axe);
				Item* sword = Item::CreateItem(3330, 1);
				realContainer->internalAddThing(sword);
				Item* club = Item::CreateItem(3310, 1);
				realContainer->internalAddThing(club);
			}
			Item* shovel = Item::CreateItem(3457, 1);
			realContainer->internalAddThing(shovel);

			Item* rope = Item::CreateItem(3003, 1);
			realContainer->internalAddThing(rope);

			Item* food = Item::CreateItem(3725, 2);
			realContainer->internalAddThing(food);

			ReturnValue ret = g_game.internalPlayerAddItem(player, container);

			std::ostringstream ss;
			ss << "* voice * You received the beginners equipment. The Tibia GODS wish you a good way in your journey - The Oracle";
