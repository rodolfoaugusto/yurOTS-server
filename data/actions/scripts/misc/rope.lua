...
function onUse(player, item, fromPosition, target, toPosition)
	...
	elseif table.contains(holeSpots, tile:getGround():getId()) or target:getId() == 435 then
		...
		if thing:isCreature() then
			local tilePz = Tile(player:getPosition())
			if tilePz:hasFlag(TILESTATE_PROTECTIONZONE) then
	        		player:sendCancelMessage("You cannot use rope inside the protection zone.")
			else
